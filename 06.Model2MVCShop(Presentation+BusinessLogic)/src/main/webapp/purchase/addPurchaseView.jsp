...
<table width="600" border="0" cellspacing="0" cellpadding="0"	align="center" style="margin-top: 13px;">
	...
	<tr>
		<td width="104" class="ct_write">쿠폰</td>
		<td bgcolor="D6D6D6" width="1"></td>
		<td class="ct_write01">
			<select id="userCheck" name="userCheck" class="ct_input_g" 
							style="width: 100px; height: 19px" maxLength="20">
			</select>
		</td>
	</tr>
...
<script type="text/javascript">
	...
	coupon()
	function coupon(){
		for(let i=1; i<=${user.coupon.size()}; i++){
			let name;
			if (${user.coupon.get(i).salePercent}=== 1){
				name="10% 쿠폰"
			} else if(${user.coupon.get(i).salePercent} ===2){
				name="20% 쿠폰"
			} else {
				name ="없음"
			}
	   		document.getElementById('userCheck').options[i-1] = new Option(name, ${user.coupon.get(i).couponNo});
	   }
	}
	//*/
</script>